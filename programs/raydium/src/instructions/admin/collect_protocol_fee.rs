...
use crate::state::events::CollectProtocolFeeEvent;
...
pub fn collect_protocol_fee(
    ...
    require!(
        pool.protocol_fees_a > 0 || pool.protocol_fees_b > 0,
        AmmError::NoFeesToCollect
    );

    let withdraw_a = amount_a_requested.min(pool.protocol_fees_a);
    let withdraw_b = amount_b_requested.min(pool.protocol_fees_b);
...
pub struct CollectProtocolFee<'info> {
    #[account(
        constraint = owner.key() == amm_config.protocol_owner || owner.key() == amm_config.admin @ AmmError::UnauthorizedProtocolOwner
    )]
